...
{ DO NOT use packrecords c here. Modern zlib (and system C libs) uses default alignment (usually 8-byte on 64-bit).
  Forcing packrecords c makes the record size smaller than the C equivalent, causing Z_VERSION_ERROR. }
// {$packrecords c} 
...
*)
  ...
  gz_header = record
    ...
    comment   : pBytef;            // pointer to zero-terminated comment or Z_NULL (Fixed: was Bytef)
...
const
  ...
  Z_FINISH = 4;
  Z_BLOCK = 5;
  Z_TREES = 6; // Added in newer versions, though less used in basic bindings
...
function inflatePrime(var strm: z_stream; bits, value: cint): cint; cdecl; external libz name 'inflatePrime';
(*
...
function gzprintf(thefile:gzFile; format:PAnsiChar): cint; cdecl; varargs; external libz name 'gzprintf';
(*
...
function gzungetc(thefile:gzFile; c:AnsiChar):AnsiChar; cdecl; external libz name 'gzungetc';
(*
